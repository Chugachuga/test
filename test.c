...
/*   Updated: 2016/03/10 17:11:55 by gvilmont         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		lastjoin(char buff[BUFF_SIZE + 1], char **line)
{
	...
	dst = ft_strsub(buff, 0, ft_strchr(buff, '\n'));
	line[0] = ft_strjoin(line[0], dst, 3);
	dst = ft_strsub(buff, ft_strchr(buff, '\n') + 1, BUFF_SIZE);
...
static int		ft_read(int const fd, char **line)
{
	...
	{
		line[0] = ft_strjoin(line[0], buff[fd], 1);
